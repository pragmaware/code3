//=============================================================================
//
//   File : C3TextEditorModeParserRust.java
...
C3TextEditorModeParserRust::C3TextEditorModeParserRust(C3TextEditorModeParserState * pParserState)
	: C3TextEditorModeParser(pParserState),
	m_pSharedData(pParserState->pMode->factory()->modeRustSharedData()),
...
C3TextEditorModeParserRust::~C3TextEditorModeParserRust()
{
}
...
void C3TextEditorModeParserRust::computeBlocksParseIdentifier()
{
	...
	if((m_p->p < m_p->e) && (m_p->p->unicode() == (ushort)'!'))
	{
		bMacro = true;
		m_p->p++;
	}
	...
	if(bMacro)
	{
		pBlock->pColor = &(m_p->pCoreData->pOptions->oSpecialFunctionTextColor);
		return;
	}
...
void C3TextEditorModeParserRust::computeMetadata()
{
	if(m_p->p >= m_p->e)
	{
	...
	if(m_p->uInterLineFlags & InRustMultiLineComment)
	{
		computeBlocksParseCLikeMultiLineComment(InRustMultiLineComment);
		if(m_p->p >= m_p->e)
			return;
		// no longer in multi line comment
		Q_ASSERT(!(m_p->uInterLineFlags & InRustMultiLineComment));
	}
	...
	while(m_p->p < m_p->e)
	{
		...
		switch(m_p->p->unicode())
		{
			case '/':
			{
				...
				{
					case '*':
						m_p->uInterLineFlags |= InRustMultiLineComment;
						m_p->p++;
					...
					case '/':
						...
						if(m_p->p >= m_p->e)
							return;
						m_p->uInterLineFlags |= InRustSingleLineComment;
			...
			break;
			case '\'':
				m_p->uInterLineFlags |= InRustString;

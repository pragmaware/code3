//=============================================================================
//
//   File : C3TextEditorModeParserASM.cpp
...
C3TextEditorModeParserASM::C3TextEditorModeParserASM(C3TextEditorModeParserState * pParserState)
	: C3TextEditorModeParser(pParserState),
	m_pSharedData(pParserState->pMode->factory()->modeASMSharedData()),
...
C3TextEditorModeParserASM::~C3TextEditorModeParserASM()
{
}
...
// FIXME: This is duplicated in CPP parser
void C3TextEditorModeParserASM::computeBlocksParseIncludeFile()
{
	...
	while(m_p->p < m_p->e)
	{
		...
		switch(m_p->p->unicode())
		{
			case '/':
			{
				...
				{
					case '*':
						m_p->uInterLineFlags |= InASMMultiLineComment;
						m_p->p++;
						BLOCK(b,m_p->p - b,oMultiLineCommentTextColor,0);
						if(m_p->p >= m_p->e)
							return;
						computeBlocksParseCLikeMultiLineComment(InASMMultiLineComment);
					break;
...
process_include:
	...
	{
		case '<':
			m_p->uInterLineFlags |= InASMIncludeFile;
			...
			case '/':
			{
				...
				switch(m_p->p->unicode())
				{
					...
					case '/':
						...
						if(m_p->p >= m_p->e)
							return;
						m_p->uInterLineFlags |= InASMSingleLineComment;
...
void C3TextEditorModeParserASM::computeBlocksParseIdentifier()
{
	...
	if(m_bInPreprocessorLine)
	{
		if(m_bFirstPreprocessorTokenSeen)
		{
			...
			pBlock->uFlags |= InASMPreprocessorToken;
...
void C3TextEditorModeParserASM::computeMetadata()
{
	if(m_p->uInterLineFlags & InASMPreprocessor)
	{
		// The InASMPreprocessor state is local. We set it only at an explicit line continuation.
		...
		m_p->uInterLineFlags &= ~InASMPreprocessor;
	...
	if(m_p->uInterLineFlags & InASMMultiLineComment)
	{
		computeBlocksParseCLikeMultiLineComment(InASMMultiLineComment);
		if(m_p->p >= m_p->e)
			return;
		// no longer in multi line comment
		Q_ASSERT(!(m_p->uInterLineFlags & InASMMultiLineComment));
	}
		...
		for(;;)
		{
			...
			switch(m_p->p->unicode())
			{
				...
				case '#':
					...
					// not a letter... assume it's a comment (yes.. GAS syntax sux)
					m_p->p++;
					BLOCK(b,m_p->p - b,oSingleLineCommentTextColor,0);
					if(m_p->p >= m_p->e)
						return;
...
preprocessor_check_complete:
	...
	while(m_p->p < m_p->e)
	{
		...
		switch(m_p->p->unicode())
		{
			...
			break;
			case '\'':
				m_p->uInterLineFlags |= InASMString;
			...
			case '\\':
				...
				if(m_p->p >= m_p->e)
				{
					// line continuation
					if(m_bInPreprocessorLine)
						m_p->uInterLineFlags |= InASMPreprocessor; // so it will be set in the line continuation block
			...
			break;
			//case '#':
			// For some GAS targets this is the beginning of a single line comment... bleargh
			//	m_p->p++;
			...
			case '.':
			{
				...
				BLOCK(b,m_p->p - b,oKeywordTextColor,0); // directive
			}
			break;
			default:
			{
